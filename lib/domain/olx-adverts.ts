...
export interface AdvertInsight {
  advert: OlxAdvert;
  messages: number; // §OLX liczba rozmów OLX przypisanych do tego ogłoszenia (kontakt = mocny lead)
  daysToExpiry: number | null;
  expired: boolean;
  expiringSoon: boolean; // ≤ 3 dni
  conversion: number | null; // (telefony + wiadomości) / wyświetlenia
...
export interface FleetSummary {
  ...
  totalViews: number;
  totalPhones: number;
  totalMessages: number; // suma rozmów OLX przypisanych do ogłoszeń
  avgViews: number;
  avgConversion: number; // średnia ważona ((telefony + wiadomości) / wyświetlenia)
...
export function summarize(adverts: OlxAdvert[], messagesByAdvert: Record<string, number> = {}): FleetSummary {
  ...
  const totalMessages = adverts.reduce((s, a) => s + (messagesByAdvert[a.olx_id] || 0), 0);
  return {
    ...
    // Skuteczność = kontakty (telefony + wiadomości) / wyświetlenia.
    avgConversion: totalViews > 0 ? (totalPhones + totalMessages) / totalViews : 0,
    toReact: 0, // uzupełniane po analizie
  };
}

export function analyzeAdvert(a: OlxAdvert, fleet: FleetSummary, messages = 0, now = Date.now()): AdvertInsight {
  ...
  // Skuteczność = (telefony + wiadomości) / wyświetlenia — wiadomość to najmocniejszy sygnał kontaktu.
  const conversion = a.total_views > 0 ? (a.total_phones + messages) / a.total_views : null;
  ...
  return { advert: a, messages, daysToExpiry, expired, expiringSoon, conversion, deltaViews, deltaPhones, recommendations: rec, priority, score: 0, rank: 0 };
}
...
function scoreAdvert(i: AdvertInsight, max: { leads: number; views: number; conv: number }): number {
  // Lead = realny kontakt: odsłona telefonu + rozmowa (wiadomość). To waży najwięcej.
  const leads = i.advert.total_phones + i.messages;
  const leadsN = max.leads > 0 ? leads / max.leads : 0;
  ...
  const raw = 0.55 * leadsN + 0.3 * convN + 0.15 * viewsN;
  return Math.round(raw * 100);
}

export function analyzeFleet(adverts: OlxAdvert[], messagesByAdvert: Record<string, number> = {}, now = Date.now()): { insights: AdvertInsight[]; summary: FleetSummary } {
  const summary = summarize(adverts, messagesByAdvert);
  const insights = adverts.map((a) => analyzeAdvert(a, summary, messagesByAdvert[a.olx_id] || 0, now));

  // Ranking: normalizacja do najlepszego w flocie, potem sort malejąco po score.
  const max = {
    leads: Math.max(0, ...insights.map((i) => i.advert.total_phones + i.messages)),
  ...
  insights.sort((x, y) => y.score - x.score || (y.advert.total_phones + y.messages) - (x.advert.total_phones + x.messages) || y.advert.total_views - x.advert.total_views);
