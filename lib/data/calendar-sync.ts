...
function tentSizeCode(size: string | null): "M" | "D" {
  if (!size) return "M";
...
function tentsCode(sizes: (string | null)[]): string {
  const codes = sizes.filter((s): s is string => Boolean(s)).map(tentSizeCode);
  codes.sort((a, b) => (a === b ? 0 : a === "M" ? -1 : 1));
  return codes.join("");
}

function cityFrom(location: string | null, customerCity: string | null): string {
...
export async function syncReservationToCalendar(reservationId: string): Promise<void> {
  ...
  let colorId: string;
  if (isIclub) {
    const code = tentsCode(tentList.map((t) => t.size));
    const city = cityFrom(r.location, r.customer?.city ?? null);
    const head = [code, r.package?.name].filter(Boolean).join(" ");
    summary = [head, city].filter(Boolean).join(" - ") || r.customer?.name || "Rezerwacja iClub";
    colorId = code.includes("D") ? COLOR_FLAMINGO : COLOR_GRAPE;
  } else {
    const items = (r.rental_items ?? "").trim();
    summary = !items || items.includes(",") ? "Wynajem sprzętu" : items;
    colorId = COLOR_SAGE;
  }
