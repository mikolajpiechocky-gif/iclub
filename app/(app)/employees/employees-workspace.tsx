      ...
      <section className={selected ? "block" : "hidden md:block"}>
        {selected ? (
          <div>
            ...
            <EmployeeSettlements key={`s-${selected.id}`} profileId={selected.id} rows={settlements[selected.id] ?? []} reviewBonus={Number(selected.rate?.review_bonus ?? 20) || 0} reelBonus={Number(selected.rate?.reel_bonus ?? 50) || 0} paidOut={Number(selected.rate?.paid_out ?? 0) || 0} />
          </div>
        ) : (
